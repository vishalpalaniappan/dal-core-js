let clpFfiJsModuleInitPromise;

if (typeof window === "undefined") {
    clpFfiJsModuleInitPromise = import("clp-ffi-js/node");
} else {
    clpFfiJsModuleInitPromise = import("clp-ffi-js");
}

/**
 * Returns the promise of the initialized CLP FFI JS module.
 * This avoids top level await and allows the module to be imported
 * in both node and browser environments. However, I might change
 * this in the future to export separate builss for node and browser.
 * @returns {Promise} Promise of the initialized CLP FFI JS module.
 */
export async function getClpFfiJsModuleInit() {
    return clpFfiJsModuleInitPromise;
}
...
class TraceDebugger {
    ...
    async loadTrace () {
        ...
        const Module = await (await getClpFfiJsModuleInit()).default();
